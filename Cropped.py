...
image = img.imread('Parrots-680x680.jpg')
#print(image.max)
plt.figure(1)
...
y=47*4
plt.figure(2)
img_cropped = image[y:(y+16),x:(x+16), :]
imgplot = plt.imshow(img_cropped)
plt.savefig('Cropped Image.jpg')
plt.title("Cropped Image - E/17/047")
#print(img_cropped)
...
quantized_img = np.digitize(img_cropped, np.arange(0, 256, 256/8)) - 1  #quantized the image


#plot the quantized image
plt.figure(3)
plt.title("Quantized Image(Cropped) - E/17/047")
plt.savefig('Quantized Image Cropped.jpg')
quantized_img_32=quantized_img*(256//8) 
imgplot = plt.imshow(quantized_img_32)

#convert to 1D 
quantized_img_1d = quantized_img.reshape((1, -1))[0]
f = open("quantized_img_1d.txt", "a")
...
Cropped_compressed_image = ''.join([str(item) for item in encoding_output]) 

############################################################
#Step 08
# write the compressed data string to a text file
with open('Cropped_compressed_image.txt', 'w') as file:
    file.write(Cropped_compressed_image)
#print(Cropped_compressed_image)

##################################################################
#Step 10
with open('Cropped_compressed_image.txt', 'r') as file:
...
decoded_pixels = np.array(decoded_data).reshape((16, 16, 3))
#print(decoded_pixels)

plt.figure(5)
plt.title(" Decompress Image(Cropped) - E/17/047")
decode_img_32=decoded_pixels*(256//8) 
imgplot = plt.imshow(decode_img_32)
plt.savefig('Decompress Image Cropped.jpg')
print(codewords)
...
print("Cropped Image Entropy: ",entropy)
